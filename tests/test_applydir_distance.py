import pytest
from applydir.applydir_distance import levenshtein_distance, levenshtein_similarity
...
def test_levenshtein_similarity():
    ...
    assert levenshtein_similarity(["line1", "line2"], ["line1", "line3"]) == pytest.approx(0.909, abs=0.001)  # Distance 1 over max len 11
    assert levenshtein_similarity([], []) == 1.0
    assert levenshtein_similarity(["a"], []) == 0.0  # Length mismatch
    assert levenshtein_similarity(["line1\nline2"], ["line1\nline3"]) == pytest.approx(0.909, abs=0.001)  # Distance 1 over max len 11